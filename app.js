...
const Container = (props) => {
	function removePlanet(elementNum) {
		const card = document.getElementsByClassName('card');
		card[elementNum].classList.add('removed');
	}
...
const Planet = (props) => {
	return (
		<div className='card'>
			<div>
				<img src={`./${props.url}`} alt={props.planetName} />
			</div>
			...
			<button className='btnRemove' onClick={props.remove}>
				Delete
			</button>
		</div>
	);
